...
class Local {
    ...
    public function getLocalAdmin($objSqlAdmin, $idLocal){
        $arrDadosFinal = array();
        $sqlBusca = "SELECT cl.*, cid.nome AS 'cidade_g', cid.estado, 
        est.nome AS 'estado_g', (SELECT COUNT(mi.id) FROM motor_ingresso mi WHERE mi.cliente = cl.id) AS 'motor_ingresso',
    ...
    public function getLocalTerceiro($objSqlAdmin, $idLocal, $objSqlCliente){
        $fornecedorAdmin = 0;
        $arrDadosFinal = array();
        ...
        if(isset($arrDados[0]['id']) && (int)$arrDados[0]['fornecedor'] > 0){
