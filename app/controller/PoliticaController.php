...
class PoliticaController extends Controller
{
    ...
    public function politica()
    {
        ...
        if((int)$cliente_admin > 1){
            ...
            $politicaHtml = isset($arrPersonalizacao[0]['descricao']) && $arrPersonalizacao[0]['descricao'] != '' ? $arrPersonalizacao[0]['descricao'] : $politicaDefault;
        } else {
            $politicaHtml = $politicaDefault;
        }

        $values = array(
            'estrutura' => array(
                'url'    => $this->url,
                'title'  => $confDominio[0]['titulo'] . ' - Política de Privacidade',
                'version' => date('YmdHis'),
                'pagina' => 'quemsomos',
                'favicon' => $faviconHtml,
        ...
        return $this->twig->render('politica-de-privacidade.html', $values);
    }
}